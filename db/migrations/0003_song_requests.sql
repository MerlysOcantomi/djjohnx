...
CREATE TABLE IF NOT EXISTS song_request_rounds (
 ...
 capacity INTEGER NOT NULL CHECK(capacity BETWEEN 1 AND 1000), reserved_slots INTEGER NOT NULL DEFAULT 0 CHECK(reserved_slots>=0 AND reserved_slots<=capacity), reopen_status TEXT NOT NULL DEFAULT 'open' CHECK(reopen_status IN ('open','last_round')),
...
CREATE TABLE IF NOT EXISTS song_requests (
 id UUID PRIMARY KEY, round_id UUID NOT NULL REFERENCES song_request_rounds(id), public_reference TEXT NOT NULL CHECK(public_reference ~ '^MC-[0-9]{4}$'),
 ...
 payment_reported_at TIMESTAMPTZ, paid_at TIMESTAMPTZ, confirmed_by TEXT, expires_at TIMESTAMPTZ NOT NULL, created_at TIMESTAMPTZ NOT NULL DEFAULT now(), updated_at TIMESTAMPTZ NOT NULL DEFAULT now(), UNIQUE(round_id,public_reference)
);
...
CREATE TABLE IF NOT EXISTS song_request_rate_limits (ip_hash CHAR(64) PRIMARY KEY, window_started_at TIMESTAMPTZ NOT NULL, request_count INTEGER NOT NULL CHECK(request_count BETWEEN 1 AND 100000), expires_at TIMESTAMPTZ NOT NULL);
...
CREATE INDEX IF NOT EXISTS idx_song_rate_limits_expiry ON song_request_rate_limits(expires_at);
COMMIT;
